...
// cds.lpData = TEXT("C:\\Folder");
// cds.cbData = size of cds.lpData in bytes including the null terminator.
// BOOL exists = (BOOL)SendMessage(everything_hwnd,WM_COPYDATA,(WPARAM)(HWND)notify_hwnd,(LPARAM)(COPYDATASTRUCT *)&cds);
...
// The Everything IPC Server enters a modal loop, preventing the database from updating and processing IPC calls only.
// this will make sure that a result_index refers to the same item between IPC calls.
// The db must be unlocked as soon as possible.
// there must be a matching EVERYTHING_IPC_WM_QUERY_UNLOCK for each successful EVERYTHING_IPC_WM_QUERY_LOCK call.
// SendMessage(everything_hwnd,EVERYTHING_IPC_WM_QUERY_LOCK,0,0); 
// returns nonzero if successful. returns 0 if not supported.
// requires Everything 1.4.1.833
